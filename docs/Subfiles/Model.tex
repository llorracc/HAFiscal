...
A consumer $i$ has education $e(i)$ and a subjective discount factor $\beta_i$. The consumer faces a stochastic income stream, $\yLvl_{i,t}$, and chooses to consume some of that income when it arrives (the `splurge') and then to optimize consumption with what is left over. Therefore, consumption each period for consumer $i$ can be written as
...
\end{verbatimwrite}
\input{\EqDir/splurge}
where $\cLvl_{i,t}$ is total consumption, $\cLvl_{sp,i,t}$ is the splurge consumption, and $\cLvl_{opt,i,t}$ is the consumer's optimal choice of consumption after splurging. Splurge consumption is simply a fraction of income:
\begin{align}
...
\begin{align}
  \mathbf{a}_{i,t} &= \mathbf{m}_{i,t} - \cLvl_{i,t}, \notag \\ 
...
\end{align}
The series $\yLvl_{AD,i,t}$ is then used for each consumer's budget constraint.
